...
def pacientes(request):
    if request.method == "GET":
        # somente os pacientes da nutricionista logada
...
def dados_paciente(request, id):
    ...
    if not paciente.nutri == request.user:
        messages.add_message(request, constants.ERROR, 'Você não tem acesso a esse paciente')
    ...
    if request.method == "GET":
        ...
        if (len(peso.strip()) == 0) or (len(altura.strip()) == 0) or (len(gordura.strip()) == 0)  or (len(musculo.strip()) == 0):
            messages.add_message(request, constants.ERROR, 'Todos os dados do paciente devem ser preenchidos')
        ...
        if (len(hdl.strip()) == 0) or (len(ldl.strip()) == 0) or (len(colesterol_total.strip()) == 0)  or (len(triglicerídios.strip()) == 0):
            messages.add_message(request, constants.ERROR, 'Todos os Dados laboratoriais devem ser preenchidos')
            return redirect(f'/dados_paciente/{id}') 
